                ...
                <div className="mt-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {posts.map((post, i) => (
                        <div key={i} className="group bg-white rounded-[2.5rem] border border-gray-100 overflow-hidden shadow-sm hover:shadow-xl transition-all duration-500">
                            ...
                            <div className="p-8">
                                <span className="text-[10px] font-black uppercase tracking-widest text-brand-red">{post.date}</span>
                                <h3 className="font-black text-xl mt-2 mb-4 uppercase tracking-tighter leading-tight">{post.title}</h3>
                                <p className="text-gray-500 text-sm leading-relaxed mb-6">{post.excerpt}</p>
                                <Link href="#" className="font-black text-xs uppercase tracking-widest text-black hover:text-brand-red transition-transform">Read More →</Link>
                            </div>
                        </div>
                    ))}
                </div>
