...
export function ProductCard({
  ...
  return (
    <div className="bg-white rounded-xl sm:rounded-[2rem] p-1.5 sm:p-2.5 border border-gray-100 hover:shadow-xl transition-all duration-500 group">
      <Link href={`/product/${id}`}>
        ...
        <div className="relative overflow-hidden rounded-lg sm:rounded-[1.5rem] aspect-[4/3] sm:aspect-square flex items-center justify-center">
          ...
          <div className="absolute inset-x-1.5 top-1.5 sm:inset-x-2 sm:top-2 flex justify-between items-start">
            <div className="bg-white/80 backdrop-blur-sm px-1.5 sm:px-3 py-0.5 sm:py-1 rounded-full text-[8px] sm:text-[10px] font-bold text-gray-700 shadow-sm border border-black/5">
              {badge || 'Best Seller'}
            </div>
            <button className="w-6 h-6 sm:w-8 sm:h-8 bg-white rounded-full flex items-center justify-center shadow-md hover:scale-110 transition-transform duration-300">
          ...
          <div className="absolute bottom-1.5 left-1/2 -translate-x-1/2 flex gap-1 z-10">
            <div className="w-1 h-1 sm:w-1.5 sm:h-1.5 rounded-full bg-brand-red" />
        ...
        <div className="mt-2 sm:mt-4 px-0.5 sm:px-1 pb-0.5 sm:pb-1">
          ...
          <div className="flex items-center gap-1 sm:gap-2 mb-1.5 sm:mb-4">
            <span className="text-xs sm:text-lg font-bold text-[#1A1A1A]">${price}</span>
            {originalPrice && (
          ...
          <Button
            className="w-full bg-[#2B2B2B] hover:bg-black text-white font-bold py-1 sm:py-2.5 h-7 sm:h-10 text-[9px] sm:text-sm rounded-full transition-all duration-300 active:scale-95 shadow-sm"
          >
            Buy Now
          </Button>
        </div>
